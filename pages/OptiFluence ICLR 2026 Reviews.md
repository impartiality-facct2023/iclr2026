## Responses
	...
	- We have not. Our only goal is to produce input canaries that fit the DP definition. "Visual naturalness" is within our desiderata and thus we are unsure it is metric worth optimizing for. Afterall, the threat model is different from an attack using adversarial example so "bounding the (visual) perturbation" is not a requirement.
