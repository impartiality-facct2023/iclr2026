...
- ### Reviewer_5zJV
	...
	- #### Weaknesses
		...
		- Our statement of scalability should be put into proper context of the baselines we present. The first being truly unrolled (exact) canary gradient, which we dubbed Unrolled-OPT (Section 5.2). These are indeed memory intensive, as we point out in Line 304, since the computational graph necessary to calculate them grows with the number of training steps. One of the novelties of our work is making unrolled gradients computationally feasible through rematerialization (trade-off memory with time) and  truncation (trade-off )
