# Responses
	...
	- > The claim in Section 6.3 that unrolled optimization yields “exact gradients” appears overstated, since ReMat+TBPTT introduces gradient truncation and therefore only provides approximate updates.
	...
	- Therefore, **we treat $k$ as not ML hyper-parameter that needs to be tuned.** Our suggestion to the practitioners is to use the largest $k$ that allows them to train accurate models with the memory they have access to; since larger $k$ by definition indicates a more accurate canary gradient; but that additional accuracy may not be necessary to achieve highly detectable canaries.
-
