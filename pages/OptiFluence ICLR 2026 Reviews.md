...
- ### Reviewer_VkF8
	...
	- 8. The method involves repeated model retraining with unrolled optimization, rematerialization, and truncated backpropagation—all computationally demanding procedures. However, the paper does not report runtime, GPU memory usage, or total training cost. Without this information, it is difficult to assess whether the framework is truly scalable beyond small benchmark datasets.
		- #response
		- To answer the reviewer’s comment regarding empirical evidences for efficiency of our method, we have updated Section 6.2 to report wall-time clock and VRAM usage. We have also included a paragraph explaining why our method scales effectively from a toy dataset such as MNIST to a more complex dataset like CIFAR-100. The latter is important because we were able to reduce our truncation parameter $k$ for TBPTT(which controls how many training steps are  included in the canary gradient calculation—see Figure 2(c)) from 4 to 2; and still managed to achieve near-perfect detectability on CIFAR-100 as well.
	- Minor comments:
