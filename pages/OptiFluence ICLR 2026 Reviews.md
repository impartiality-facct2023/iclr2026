## Responses
	...
	- > W2. Only report TPR @ Low FPR; other metrics such as AUC should also be considered.
		...
		- The reason we do not already report AUC numbers is that prior work [1] shows that AUC "is often uncorrleated with low false-positive success rates." AUC is considered an average-case success attack metric for the attacker, while TPR@lowFPR is a more stringent worst-case metric. Finally worst-case TPR, FPR numbers have a direct relationship with privayc parameter lowerbounds $\varepsilon_{-}$ (see our response to Reviewer VkF8's).  Therefore, we chose to report the worst-case metric.
