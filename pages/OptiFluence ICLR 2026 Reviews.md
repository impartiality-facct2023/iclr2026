...
- ### Reviewer_VkF8
	...
	- 5. Since the framework is inspired by differential privacy but ultimately empirical, can you clarify how the metric (TPR@FPR) relates to formal ε or δ values? Is there any attempt to estimate lower bounds on ε or compare to DP auditing baselines that produce numeric privacy budgets?
		...
		- We nevertheless have included DP-Auditing results in Section 6.2 with the TPR@0.1FPR metric to show where first-party tight audits are not possible optimized canaries can provide useful alternative. Using the privacy-estimates`` by Zanella-Béguelin et al, we have calculated
		-
