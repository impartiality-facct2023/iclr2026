## Responses
	...
	- ### Reviewer_5uPC
		...
		- > The approximation approach ReMat+TBPTT is proposed but there seem to be no results that show why you should use it over the fully unrolled updates.
			- We do in fact motivate the necessity of ReMat+TBPTT at the start of Section 5.3. The (exact) unrolled gradients of Section 5.2 (the Unrolled-OPT baseline) are so memory intensive that one could not fit the computational graph for any reasonable ResNet  CIFAR10 model on  model to fit in memory; while we could train a Resnet50 and WideResNet model using the ReMat+TBPTT approach.
			-
