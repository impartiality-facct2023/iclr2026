### Rebuttal Summary {{renderer :wordcountchar_}}
	...
	- **Input-space vs. weight-space canaries**: We clarified that while prior optimization-based work (Maddock et al., Nasr et al.) may also take place in the input-space; their *strongest* canaries are released in the weight-space (federated learning, canary gradients). OptiFluence however exclusively generates input-space canaries enabling architecture-agnostic, transferable third-party auditing—a fundamentally different threat model and deployment context.
